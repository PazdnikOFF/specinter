...
# Синонимы поиска: группы взаимозаменяемых терминов (RU-сленг/сокращения запчастей).
# Только БЕЗОПАСНО-эквивалентные (сокращения и порядок слов), без спорных обобщений.
SYNONYM_GROUPS = [
    ["сайлентблок", "сайлент-блок", "сайлент блок", "с/б"],
    ["турбокомпрессор", "турбина", "ткр"],
    ["коленвал", "коленчатый вал", "вал коленчатый"],
    ["распредвал", "распределительный вал", "вал распределительный"],
    ["гидроцилиндр", "гидравлический цилиндр", "цилиндр гидравлический", "г/ц"],
    ["гидронасос", "гидравлический насос", "насос гидравлический"],
    ["рвд", "рукав высокого давления", "шланг высокого давления"],
    ["ремкомплект", "рем комплект", "рем-комплект", "р/к"],
    ["подшипник", "п/ш"],
    ["фильтр", "фильтрующий элемент", "фильтроэлемент"],
    ["прокладка", "уплотнение", "уплотнительная прокладка"],
    ["форсунка", "распылитель форсунки"],
    ["амортизатор", "аморт", "амморт"],
    ["наконечник рулевой", "рулевой наконечник"],
]


def _synonyms() -> dict:
    """Разворачивает группы в двунаправленный словарь Meili {term: [прочие в группе]}."""
    out: dict[str, list[str]] = {}
    for grp in SYNONYM_GROUPS:
        for w in grp:
            out[w] = [x for x in grp if x != w]
    return out
