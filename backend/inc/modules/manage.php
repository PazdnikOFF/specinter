...
class manage {
	...
	private function autocomplete() {
		...
		$list = new Listing("catitem", "blocks", "all", "name LIKE '$query%' AND ");
		$list->getList();
		$list->getItem();
		$item = $list->item;
		...
		foreach ($item as $key => $val) {
			...
			$newItem->value = $val->name;
			$newItem->data = "/manage/blockedit/_aedit_id".$val->id."_templatecatitem_parent".$val->parent."/";

			array_push($newItems, $newItem);
		}

		$object = new StdClass();
		$object->suggestions = $newItems;
