...
class sys_images
{
    ...
    public function __construct($fileName)
    {
        ...
        if (!file_exists($this->fileName)) {
            $this->errmsg = 'Файл не найден ' . $this->fileName;
            $this->error = true;
        } //check to see if file is readable
        elseif (!is_readable($this->fileName)) {
            $this->errmsg = 'Файл не доступен для чтения';
            $this->error = true;
        }
        ...
        if ($this->error == false) {
            ...
            //unknown file format
                ...
                //debug($this->fileName);
                $this->errmsg = 'Неизвестный формат файла';
                $this->error = true;
            }
        }
        ...
        if ($this->error == false) {
            switch ($this->format) {
                ...
                case 'PNG':
                    ...
                    ImageAlphaBlending($this->oldImage, false);
    ...
    public function cropFromCenterDif($cropX, $cropY)
    {
        if ($cropX > $this->currentDimensions['width']) $cropX = $this->currentDimensions['width'];
        if ($cropY > $this->currentDimensions['height']) $cropY = $this->currentDimensions['height'];

        $cropXNew = intval(($this->currentDimensions['width'] - $cropX) / 2);
        $cropYNew = intval(($this->currentDimensions['height'] - $cropY) / 2);

        if (function_exists("ImageCreateTrueColor")) {
            $this->workingImage = ImageCreateTrueColor($cropX, $cropY);
        } else {
            $this->workingImage = ImageCreate($cropX, $cropY);
        }
     ...
     * @param string $name
     */
    public function show($quality = 100, $name = '', $watermark = false)
    {
        switch ($this->format) {
            case 'GIF':
                if ($name != '') {
    ...
    public function watermark($imagePath)
    {
        ...
        $watermarkPath = DOC_ROOT . '/img/watermark.png';
     ...
     * @param int $quality
     */
    public function save($name, $quality = 100, $watermark = false)
    {
        $this->show($quality, $name, $watermark);
    }
